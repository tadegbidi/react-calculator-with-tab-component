...
export default function App() {
  ...
  function handleNumbers(e) {
    ...
    switch(val) {
      ...
      case "c": 
        ...
        break;
      case ".": 
      if(point > 2 || String(displayer).includes(".") && !String(displayer).includes(" ")) return;
      setPoint(p => p += 1);
      // if(point >= 2) return;
      setDisplayer(x => x = x + val);
        break;
      default:
        (displayer === 0) ? setDisplayer(x => Number(x) + Number(val)) : setDisplayer(x => x += val);
        return;
    }
  ...
  return (
    <>
      <div id="calculator">
        <header><Displayer display={displayer} /></header>
        <main>
      ...
      <div id="accordion">
        <ul id="tabs">
          {infos.map(el => <li key={el.id} className={el.id === tab ? 'active' : '' } onClick={() => handleTab(el.id)}>{el.title}</li>)}
        </ul>
        <div id="tabs-content">
          {infos.map(el => el.id === tab && <p key={el.id}>{el.description}</p>)}
        </div>
      </div>
    </>
  )
